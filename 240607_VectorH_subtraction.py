...
fl.next('show subtracting plots')
filenames_subtract = (
    '240607_VectorH_Uncoupled_wout-helix.fld', #no helix
    '240607_VectorH_Uncoupled_w-helix.fld', #with helix
)
...
def load_hfss_subtract_vectors(no_helix_vs_helix):
    return ( load_hfss_vectors_as_nddata(no_helix_vs_helix[1])
         - load_hfss_vectors_as_nddata(no_helix_vs_helix[0]))
d = load_hfss_subtract_vectors(filenames_subtract)
for j in range(3):
    figure(j+1)
    title(f'component {j+1} residual')
    image(d['vec',j])

# In[5]:

fl.next('looking at individual fields for no helix')
u_index, u_axis, v_index, v_axis, data_no_helix = load_hfss_vectors(filenames_subtract[0])
for j in range(3):
    figure(3+j+1)
    title(f'component{j+1} no helix')
    image(data_no_helix[:,:,j])

# In[6]:

fl.next('looking at individual fields for with helix')
u_index, u_axis, v_index, v_axis, data_w_helix = load_hfss_vectors(filenames_subtract[1])
for j in range(3):
    figure(6+j+1)
    title(f'component{j+1} with helix')
    image(data_w_helix[:,:,j])
